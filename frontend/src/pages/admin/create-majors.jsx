import React from "react";
import { useState, useEffect } from "react";
import Layout from "@/components/layout";
import { Topbar } from "@/components/topbar";
import {
  FormField,
  FormDescription,
  FormControl,
  FormLabel,
  FormItem,
  FormMessage,
  Form,
} from "@/components/ui/form";
import { useForm } from "react-hook-form";
import { Input } from "@/components/ui/input";
import { yupResolver } from "@hookform/resolvers/yup";
import { validateMajorsForm } from "@/lib/validations";
import { Button } from "@/components/ui/button";
import { handleCreate } from "@/lib/commonFunctions";
import { useAuth } from "@/lib/dataContext";
export default function CreateMajors() {
  const { currentUser, token } = useAuth();
  const majorForm = useForm({
    resolver: yupResolver(validateMajorsForm),
    defaultValues: {
      name: "",
      degree: "",
      department: "",
      description: "",
          },
  });
  const onSubmit = (values) => {
    console.log("v", values);
  };

  useEffect(() => {
    // if (currentUser && currentUser?.role !== "admin") {
    //   navigate("/not-authorized");
    // }
    // if(!token){
    //     navigate('/login') 
    // }
    //
  }, [token]);

  return (
    <>
      {currentUser ? (
        <Layout user={currentUser}>
          <Topbar title="Add New Department" mode="create" />
          <div className="w-[80%] m-auto py-4">
            <Form {...majorForm}>
              <form
                onSubmit={majorForm.handleSubmit(onSubmit)}
                className="grid space-y-8"
              >
                <Button type="submit" className={"ml-auto"}>
                  Save
                </Button>
                <div className="grid gap-8">
                  <FormField
                    control={majorForm.control}
                    name="name"
                    render={({ field }) => (
                      <FormItem className={"relative"}>
                        <FormLabel>Major Name</FormLabel>
                        <FormControl>
                          <Input placeholder="Department Name" {...field} />
                        </FormControl>
                        <FormMessage
                          className={"text-xs absolute -bottom-5 left-0"}
                        />
                      </FormItem>
                    )}
                  ></FormField>

                  <FormField
                    control={majorForm.control}
                    name="track"
                    render={({ field,fieldState }) => (
                        <CreatableSelect
                          form={majorForm}
                          options={tracks}
                          error={fieldState.error}
                          controller={field}
                          onCreateOption={(inputValue) => handleCreate(client,setIndustries, setValue, inputValue, 'industry')}
                          placeholder="Select tracks..."
                          searchPlaceholder="Search industry..."
                          createLabel="Create new industry"
                          className="w-full"
                        />
                    )}
                  ></FormField>
                  <FormField
                    control={majorForm.control}
                    name="room"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Room Number</FormLabel>
                        <FormControl>
                          <Input
                            placeholder="Room Number (eg. N604)"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage
                          className={"text-xs absolute -bottom-5 left-0"}
                        />
                      </FormItem>
                    )}
                  ></FormField>
                </div>
              </form>
            </Form>
          </div>
        </Layout>
      ) : (
        <div></div>
      )}
    </>
  );
}
